...
fetch_statement
    : FETCH cursor_name 
      ...
      | BULK COLLECT INTO variable_name (',' variable_name )* (LIMIT (variable_name | numeric))?)
    ;
...
// Alris TODO: preprocessor_internal_statement может быть пустой строкой в том случае, если это не вредит компиляции
// пример, не вызывающий ошибок компиляции:
//BEGIN
