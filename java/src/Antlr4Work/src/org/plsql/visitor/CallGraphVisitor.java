...
public class CallGraphVisitor extends PlSqlRuleVisitor {
    public enum ExpressionType {
        FUNCTION, PROCEDURE, VARIABLE, UNKNOWN
    }
    ...
    private void logContext(ParserRuleContext ctx) {
        System.out.println(ctx.start.getLine() + "> " +  PlSqlUtils.getOriginalText(input, ctx));
    }
    ...
    private String getFullProcName(ParserRuleContext ctx) {
        ...
        while (c != null) {
            if (c instanceof Function_specContext) {
                ...
                fullName = PlSqlUtils.getOriginalText(input, pbc.package_name(0)) + ":PKG." + fullName;
                break;
            }
            c = c.getParent();
        }
...
//            if (!isFunctionOrProcedureSpec(ctx)) {
//                System.out.println(PlSqlUtils.getLine(ctx) + "  new " + name + " [" + ctx.getClass().toString() + "]");
//            }
//        }
